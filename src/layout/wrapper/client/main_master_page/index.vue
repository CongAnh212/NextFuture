<template>
  <div class="wrapper bg-white">
    ...
    <router-view v-if="isLoadComponentMessage" name="messages"></router-view>
    <div v-if="!isLoadComponentMessage">
    ...
    <div v-if="!isLoadComponentMessage" class="px-0 mx-0"
         style="position: absolute; right: 0; width: 79%;top: 4.25rem; background-color: #ffffff;">
      <router-view name="content" :sentFriend="dataRequestFriend" @profile_request_friend="handleProfileRequestFriend"
                   :sentFriendSuggest="dataSuggest" @profile_suggest="handleProfileSuggest"
                   :delFriendSuggest="delDataSuggest"
                   @removeNotify="handleNotify" @profile_del_friend="handleDelProfileAllFriend"
...
export default {
  ...
  watch: {
    '$route'(to, from) {
      if (to.path === '/messages') {
        this.isLoadComponentMessage = true
      }
    }
  },
  created() {
